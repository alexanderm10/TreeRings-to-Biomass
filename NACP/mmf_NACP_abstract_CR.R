...
diam.data <- read.csv("tree_data3.csv", na.strings=c("", "NA", "#VALUE!", "*"), header=T)
#adding a column include which plot at the site the trees belong to
diam.data$plot <- substr(diam.data$PlotID, 3, 3)
diam.data$plot <- as.factor(diam.data$plot)
...
mmf.tr <- read.rwl("mmf_plotbplus.rwl")
head(mmf.tr)
summary(mmf.tr)
...
head(diam.data)
dbh.bins1 <- seq(10, max(diam.data$dbh[diam.data$site == "Morgan Monroe State Park"], na.rm=T), 5) # 5 cm bins based on the range of your trees
...
diam.data$bin <- cut(diam.data$dbh, breaks=c(dbh.bins1))#we need to look into this
...
#we already have a file with all of the core data.  We can uplad that.
core.data <- read.csv("Core_data_DOE_summer_2014.csv", na.strings=c("", "NA", "#VALUE!", "*"),header = T)
summary(core.data)

#ID <- as.data.frame(c(paste(diam.data$TreeID, "A", sep=""), paste(all.valles$id, "B", sep=""), paste(all.valles$id, "C", sep="")))
#names(ID) <- "ID"

names(core.data)
core.deets <- data.frame(names(mmf.tr2)) 
names(core.deets) <- "ID"
summary(core.deets)
# checking to make sure it worked
core.deets[substr(core.deets$ID, 1, 6)=="MMB001",]

summary(core.deets)
names(core.data)

for(i in unique(core.deets$ID)){
  core.deets[core.deets$ID==i, "DBH"] <- core.data[core.data$CoreID==i,"DBH..cm."]
}
summary(core.deets)
...
mmf.tr2 <- mmf.tr2[order(row.names(mmf.tr2), decreasing=T),order(names(mmf.tr2))]
head(mmf.tr2)
...
#should we put in the pith offset estimates here and get to the center of the tree.  Flurin gave me some scripts for this, but I'm not sure what they do yet.  Haven't looked
#put the pith estimates in here!
...
for(j in seq_along(dbh.recon)){
  ...
  for(i in 2:(length(dbh.recon[,j]))){
    dbh.recon[i,j] <- ifelse(!is.na(mmf.tr2[i,j]), dbh.recon[i-1,j] - mmf.tr2[i-1,j], mmf.tr2[i,j]) # subtracting the previous year's growth from DBH to get that year's DBH
  }
}
summary(dbh.recon)
...
#setting diameter on a per hectare basis
...
diam.density <- dbh.recon

for(i in unique(trees)){
  cols <- which(substr(names(dbh.recon),1,6)==i)
  diam.density[,cols] <- dbh.recon[,cols]*diam.data[diam.data$TreeID==i,"density"]
}
summary(diam.density)
# Now the format is diameter per core on a per hectare basis
#Need diameter per TREE on a per Hectare basis
#we lost years as the row.names

##
## CRR Note: diam.tree.ha isn't worth anything; BA must be caulcated for each tree and then summed b/c of the square function: 2^2 + 2^2 is not the same as (2+2)^2 
##
diam.tree.ha <- data.frame(array(NA, dim=c(nrow(dbh.recon), length(trees))))
row.names(diam.tree.ha) <- row.names(dbh.recon)  #CRR Added
head(diam.tree.ha)


for(i in unique(trees)){
  cols <- which(substr(names(diam.density),1,6)==i)
  ifelse(length(cols) > 1, diam.tree.ha[,which(trees==i)] <- rowMeans(diam.density[,cols], na.rm=T), diam.tree.ha[,which(trees==i)] <- diam.density[,cols])
}
summary(diam.tree.ha)

#now in a diameter per TREE need Diameter per PLOT

# plots <- unique(diam.data$PlotID) - Ross's script
# Dr. Rollinson Fix:
plots <- unique(substr(names(diam.tree.ha), 1, 3)) # You had the right idea, but it was throwing errors because you were trying to evaluate plots you haven't gotten to yet
diam.plot.ha <- data.frame(array(NA, dim=c(nrow(diam.tree.ha), length(plots))))
names(diam.plot.ha) <- plots

for(i in unique(plots)){
  cols <- which(substr(names(diam.tree.ha),1,3)==i)
  ifelse(length(cols) > 1, diam.plot.ha[,which(plots==i)] <- rowSums(diam.tree.ha[,cols], na.rm=T), diam.plot.ha[,which(plots==i)] <- diam.tree.ha[,cols])
}
summary(diam.plot.ha)
...
#allometric equations for MMF samples from Jenkins database
allometric.variables <- read.csv("mmf_jenkins_eqtns.csv", header=T)
summary(allometric.variables)

#function for the Jenkins equation
jenkins.log10 <- function(dbh, a, b, c){
  10^(a + b*log10(dbh^c))
}
test<-jenkins.log10(mmf.tr2[,1], 1 , 2, 3)
summary(mmf.tr2)
summary(test)
